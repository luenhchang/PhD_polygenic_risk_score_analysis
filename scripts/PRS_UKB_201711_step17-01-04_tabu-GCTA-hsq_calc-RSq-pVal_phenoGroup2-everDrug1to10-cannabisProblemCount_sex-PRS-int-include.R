#-------------------------------------------------------------------------------------------------
# Program       : PRS_UKB_201711_step17-01-02_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup2-everDrug1to10-cannabisProblemCount_sex-PRS-int-include.R
# Modified from : PRS_UKB_201711_step17-01-01_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup2-everDrug1to10-cannabisProblemCount_sex-PRS-int-exclude.R
...
#dir.create(output.folder.path.sexPRS.exclu)
...
# Read *.hsq files 
fileList.sexPRS.inclu <- list.files(path=input.folder.path.sexPRS.inclu,pattern = ".hsq") # length(fileList.sexPRS.inclu) 400 files
...
for (i in 1:length(fileList.sexPRS.inclu)){
  fileName.sexPRS.inclu <- fileList.sexPRS.inclu[i]
  print(paste0("=================================iteration",i,"========================"))
  GCTA.output.sexPRS.inclu <- scan(file=paste0(input.folder.path.sexPRS.inclu,fileName.sexPRS.inclu),sep="\t", what="")
}

# Iteration/file 1-400 have 71 items. Items scanned from GCTA hsq, their corresponding line number in the hsq, corresponding variable in the input file for GCTA, and values in the line of hsq 
...
# 64,65 31    covar1    (wave estimate, SE)
# 66,67 32    covar1    (wave estimate, SE. levels of wave: NU2, NU3, NU1. num estimate= levels-1=2 )
# 68,69 33    covar2    (nSEX estimate, SE)
# 70,71 34    covar3    (ImpCov estimate, SE)
#--------------------------------------------------------------
...
fileList.sexPRS.inclu.keep <- grep(list.files(path=input.folder.path.sexPRS.inclu,pattern = ".hsq")
                                   ,pattern = "CUD"
                                   ,inv = TRUE
                                   ,value = TRUE) # length(fileList.sexPRS.inclu.keep) 400
...
for (i in 1:length(fileList.sexPRS.inclu.keep)){
  #i= 1 720 
  # Extract file name from the file list
  fileName <- fileList.sexPRS.inclu.keep[i]
  ...
  categoricalCovars=c("wave","wave_dup","nSEX","impCov") # wave_dup is a copy of wave
  ...
  #allFixedEffectVar=c(depVarName,allCovariates) # 21 items
