#----------------------------------------------------------------------------------------------
# Program       : PRS_UKB_201711_step17-07-02_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup7-adults-nicotine-dependence-and-more-diagnoses_sex-PRS-int-include.R
# Modified from : PRS_UKB_201711_step17-07-01_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup7-adults-nicotine-dependence-and-more-diagnoses_sex-PRS-int-exclude.R
...
# Type File
#-----------------------------------------------------------------------------------------------------
# Input Sys.glob(paste0(input.folder.path.sexPRS.exclu,"*.hsq"))
# Outpu paste0(output1,filePrefix,"part1_variance",".tsv")
# Outpu paste0(output1,filePrefix,"part2",".tsv")
# Outpu paste0(output1,filePrefix,"part3_fixedEffects",".tsv")

#------------------------------------------------------------------------------------------------------
# Sys.Date()  History
#----------------------------------------------------------------------------------------
# 2018-12-05  Exported the 3 files above
# 2018-11-24  Generated the 3 CSV files above
# 2018-09-27  Exported the 3 CSV files above
#----------------------------------------------------------------------------------------
...
input.folder.path.sexPRS.inclu <- paste0(locGCTA,"output/",phenotypeGroupName,"_sex-PRS-interact-inclu","/")

# Output folder path
output.folder.path.sexPRS.inclu <- paste0(locGCTAOut_tabulated,phenotypeGroupName,"_sex-PRS-interact-inclu","/")
...
#----------------------------------------------------------------------------------------------------
#IDRemappedPhenoFile_IDRemappedPRSFile <- "pheno7AdultsNicotineDependenceAndMore-IDremapped_standardised-IDremapped-PRS-GSCAN.txt"
IDRemappedPhenoFile_IDRemappedPRSFile <- "pheno7AdultsNicotineDependenceAndMore-IDremapped_standardised-IDremapped-PRS-GSCAN_sex-PRS-interaction.tsv"
...
ImportATabSeparatedFile(input.file.path = paste0(locPheno,IDRemappedPhenoFile_IDRemappedPRSFile)
                        ,data.name = "dataPhenoPRS") # dim(dataPhenoPRS) 8245  114

# Read *.hsq files 
fileList.sexPRS.inclu <- list.files(path=input.folder.path.sexPRS.inclu,pattern = ".hsq") # length(fileList.sexPRS.inclu) 360 files
...
#--------------------------------------------------------------------------------------------------#
count=0
for (i in 1:length(fileList.sexPRS.inclu)){
  fileName <- fileList.sexPRS.inclu[i]
  GCTAOut <- scan(file=paste0(input.folder.path.sexPRS.inclu,fileName),sep="\t", what="")
  count=count+1
  print(paste0("===============================iteration",count,"========================"))
}

# Iteration 1-360 have 67 items read. Items scanned from GCTA hsq, their corresponding line number in the hsq, corresponding variable in the input file for GCTA, and values in the line of hsq 
...
# 34,35 16    qcovar2   (sex*PRS estimate, SE)
# 36,37 17    qcovar3   (age estimate, SE)
# 38,39 18    qcovar4   (ageSq estimate, SE)
# 40,41 19    qcovar5   (sexAge estimate, SE)
# 42,43 20    qcovar6   (sexAgeSq estimate, SE)
# 44,45 21    qcovar7   (PC1 estimate, SE)
# 46,47 22    qcovar8   (PC2 estimate, SE)
# 48,49 23    qcovar9   (PC3 estimate, SE)
# 50,51 24    qcovar10  (PC4 estimate, SE)
# 52,53 25    qcovar11  (PC5 estimate, SE)
# 54,55 26    qcovar12  (PC6 estimate, SE)
# 56,57 27    qcovar13  (PC7 estimate, SE)
# 58,59 28    qcovar14  (PC8 estimate, SE)
# 60,61 29    qcovar15  (PC9 estimate, SE)
# 62,63 30    qcovar16  (PC10 estimate, SE)
# 64,65 30    covar1    (sex estimate, SE)
# 66,67 31    covar2    (ImpCov estimate, SE)
#--------------------------------------------------------------
...
for (i in 1:length(fileList.sexPRS.inclu)){
  ...
  GCTAOut <- scan(file=paste0(input.folder.path.sexPRS.inclu,fileName),sep="\t", what="") # Read 65 items
  ...
  allCovariates=c(inputFileNamePart3.3 # PRS name
                  ,paste0("sex_", inputFileNamePart3.3) # sex by PRS interaction
                  ,quantitativCovars # The rest of q covariates
                  ,categoricalCovars) # All the categorical covariates
...
#-------------------------------------------------------------------------------------#
filePrefix= "GREML1var_phenoGroup7_adults-nicotine-dependence-and-more_result_"

ExportFileTabSeparated(data=appended_part1
                       ,output.file.path=paste0(output.folder.path.sexPRS.inclu,filePrefix,"part1_variance",".tsv"))

ExportFileTabSeparated(data=appended_part2
                       ,output.file.path=paste0(output.folder.path.sexPRS.inclu,filePrefix,"part2",".tsv"))

ExportFileTabSeparated(data=appended_part3
                       ,output.file.path=paste0(output.folder.path.sexPRS.inclu,filePrefix,"part3_fixedEffects",".tsv"))

# Copy this script for similar jobs
#setwd("/mnt/backedup/home/lunC/scripts/PRS_UKB_201711/")
#file.copy("PRS_UKB_201711_step17-07-01_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup7-adults-nicotine-dependence-and-more-diagnoses_sex-PRS-int-exclude.R","PRS_UKB_201711_step17-07-02_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup7-adults-nicotine-dependence-and-more-diagnoses_sex-PRS-int-include.R")
