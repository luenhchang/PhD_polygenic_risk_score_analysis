#----------------------------------------------------------------------------------------------
# Program       : PRS_UKB_201711_step17-03-03_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup5-diagMD-diagSU_sex-PRS-int-exclude_one-sex-males.R
# Modified from : PRS_UKB_201711_step17-03-02_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup5-diagMD-diagSU_sex-PRS-int-exclude_one-sex-females.R
...
# Type File
#-----------------------------------------------------------------------------------------------------
# Input Sys.glob(paste0(input.folder.path.sexPRS.exclu.males,"*.hsq")) # 760 files
...
#-----------------------------------------------------------------------------------------------
fileList.sexPRS.exclu.males <- list.files(path=input.folder.path.sexPRS.exclu.males,pattern = ".hsq") # length(fileList.sexPRS.exclu.males) 760 files
...
males.append.part1= data.frame(phenotype = NULL
                       ,covarPheno=NULL
                       ,covariate = NULL
                       ,covarLevel= NULL
                       ,GCTAoutputFile=NULL
...
males.append.part2= data.frame(phenotype = NULL
                       ,covarPheno=NULL
                       ,covariate = NULL
                       ,covarLevel=NULL
                       ,GCTAoutputFile=NULL
                       ...
                       ,Pval=NULL
                       ,n=NULL)
males.append.part3=data.frame(phenotype = NULL
                      ,covarPheno=NULL
                      ,covariate = NULL
                      ,covarLevel=NULL
                      ,GCTAoutputFile=NULL
...
for (i in 1:length(fileList.sexPRS.exclu.males)){
  ...
  ## Check varName with names(data.pheno.group5.males)
  ...
  GCTAOut <- scan(file=paste0(input.folder.path.sexPRS.exclu.males,fileName),sep="\t", what="") # Read 61 items

  # Extract sample size as nuermic form
  sampleSize <- as.numeric(get("GCTAOut")[27]) # 959
  ...
  males.append.part1 <- rbind(males.append.part1,per_iteration_result_part1)
  ...
  males.append.part2 <- rbind(males.append.part2,per_iteration_result_part2)
  ...
  for (fixEffect in allFixedEffectVar){
    ...
    phenotype.SD <- sd(x=data.pheno.group5.males[,depVarName],na.rm= T) # Get SD of a target phenotype from phenotype file
    fixed.effect.SD <- sd(data.pheno.group5.males[,fixEffect],na.rm= T) # Get SD of the fixed effect from phenotype file
    # Calculate R2
  ...
  males.append.part3 <- rbind(males.append.part3,dfPart3)
...
#file.copy("PRS_UKB_201711_step17-03-03_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup5-diagMD-diagSU_sex-PRS-int-exclude_one-sex-males.R")
