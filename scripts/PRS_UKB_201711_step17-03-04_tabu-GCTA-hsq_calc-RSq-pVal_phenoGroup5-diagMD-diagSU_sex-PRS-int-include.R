#----------------------------------------------------------------------------------------------
# Program       : PRS_UKB_201711_step17-03-02_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup5-diagMD-diagSU_sex-PRS-int-include.R
# Modified from : PRS_UKB_201711_step17-03-01_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup5-diagMD-diagSU_sex-PRS-int-exclude.R
...
# Type File
#-----------------------------------------------------------------------------------------------------
# Input /mnt/lustre/working/lab_nickm/lunC/PRS_UKB_201711/GCTA/output/phenoGroup5_diagMD-diagSU/*.hsq
# Outpu paste0(output.folder.path.sexPRS.inclu,filePrefix,"part1_variance",".tsv")
# Outpu paste0(output.folder.path.sexPRS.inclu,filePrefix,"part2",".tsv")
# Outpu paste0(output.folder.path.sexPRS.inclu,filePrefix,"part3_fixedEffects",".tsv")
...
ImportATabSeparatedFile(input.file.path = paste0(locPheno,IDRemappedPhenoFile_IDRemappedPRSFile)
                        ,data.name = "dataPhenoPRS") # dim(dataPhenoPRS) 2327  124

# Read *.hsq files 
#filesWantList= list.files(path=locGCTAOut,pattern = ".hsq") # 840 files
fileList.sexPRS.inclu <- list.files(path=input.folder.path.sexPRS.inclu,pattern = ".hsq") # length(fileList.sexPRS.inclu) 840 files
...
# Iteration 1-840 have 69 items read. Items scanned from GCTA hsq, their corresponding line number in the hsq, corresponding variable in the input file for GCTA, and values in the line of hsq 
...
for (i in 1:length(fileList.sexPRS.inclu)){
  ...
  GCTAOut <- scan(file=paste0(input.folder.path.sexPRS.inclu,fileName),sep="\t", what="") # Read 67 items

  # Extract sample size as nuermic form
  sampleSize=as.numeric(get("GCTAOut")[27]) # 2327
...
#file.copy("PRS_UKB_201711_step17-03-02_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup5-diagMD-diagSU_sex-PRS-int-include.R")
