#----------------------------------------------------------------------------------------------
# Program       : PRS_UKB_201711_step17-07-03_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup7-adults-nicotine-dependence-and-more-diagnoses_sex-PRS-int-exclude_one-sex-males.R
# Modified from : PRS_UKB_201711_step17-07-02_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup7-adults-nicotine-dependence-and-more-diagnoses_sex-PRS-int-exclude_one-sex-females.R
...
# Type File
#-----------------------------------------------------------------------------------------------------
# Input Sys.glob(paste0(input.folder.path.sexPRS.exclu.males,"*.hsq"))
# Outpu paste0(output.folder.path.sexPRS.exclu.males,filePrefix,"part1_variance",".tsv")
# Outpu paste0(output.folder.path.sexPRS.exclu.males,filePrefix,"part2",".tsv")
# Outpu paste0(output.folder.path.sexPRS.exclu.males,filePrefix,"part3_fixedEffects",".tsv")

#------------------------------------------------------------------------------------------------------
# Sys.Date()  History
#----------------------------------------------------------------------------------------
# 20190102    Exported the 3 files above
#----------------------------------------------------------------------------------------
...
input.folder.path.sexPRS.exclu.males <- paste0(locGCTA
                                                  ,"output/"
                                                  ,phenotypeGroupName
...
# Output folder paths
output.folder.path.sexPRS.exclu.males <- paste0(locGCTAOut_tabulated
                                          ,phenotypeGroupName
                                          ,"_sex-PRS-interact-exclu"
                                          ,"/males-only/")
...
ImportATabSeparatedFile(input.file.path = paste0(locPheno,IDRemappedPhenoFile_IDRemappedPRSFile)
                        ,data.name = "dataPhenoPRS") # dim(dataPhenoPRS) 8240  114

# Read *.hsq files 
fileList.sexPRS.exclu.males <- list.files(path=input.folder.path.sexPRS.exclu.males,pattern = ".hsq") # length(fileList.sexPRS.exclu.males) 360 files
...
#--------------------------------------------------------------------------------------------------#
count=0
for (i in 1:length(fileList.sexPRS.exclu.males)){
  fileName <- fileList.sexPRS.exclu.males[i]
  GCTAOut <- scan(file=paste0(input.folder.path.sexPRS.exclu.males,fileName),sep="\t", what="")
  count=count+1
  print(paste0("===============================iteration",count,"========================"))
}

# Iteration 1-360 have 59 items read. Items scanned from GCTA hsq, their corresponding line number in the hsq, corresponding variable in the input file for GCTA, and values in the line of hsq 
...
# 58,59 28    covar1    (ImpCov estimate, SE)
#--------------------------------------------------------------
...
males.base.part1= data.frame(phenotype = NULL
                             ,covarPheno=NULL
                             ,covariate = NULL
                             ,covarLevel= NULL
                             ,GCTAoutputFile=NULL
...
males.base.part2= data.frame(phenotype = NULL
                             ,covarPheno=NULL
                             ,covariate = NULL
                             ,covarLevel=NULL
                             ,GCTAoutputFile=NULL
                             ...
                             ,Pval=NULL
                             ,n=NULL)
males.base.part3=data.frame(phenotype = NULL
                            ,covarPheno=NULL
                            ,covariate = NULL
                            ,covarLevel=NULL
                            ,GCTAoutputFile=NULL
...
for (i in 1:length(fileList.sexPRS.exclu.males)){
  ...
  GCTAOut <- scan(file=paste0(input.folder.path.sexPRS.exclu.males,fileName),sep="\t", what="") # Read 59 items

  # Extract sample size as nuermic form
  sampleSize <- as.numeric(get("GCTAOut")[27]) # 3949
  ...
  males.base.part1 <- rbind(males.base.part1,per_iteration_result_part1)
  ...
  males.base.part2 <- rbind(males.base.part2,per_iteration_result_part2)
  ...
  categoricalCovars.oneSex <- c("ImputationRun")
  
  allCovariates.oneSex <- c(inputFileNamePart3.3
                            ,quantitativCovars.oneSex
                            ,categoricalCovars.oneSex)
  ...
  allFixedEffectVar=c(depVarName,allCovariates.oneSex)
  ...
  males.base.part3 <- rbind(males.base.part3,dfPart3)
...
ExportFileTabSeparated(data=males.base.part1
                       ,output.file.path=paste0(output.folder.path.sexPRS.exclu.males
                                                ,filePrefix
                                                ,"part1_variance",".tsv"))

ExportFileTabSeparated(data=males.base.part2
                       ,output.file.path=paste0(output.folder.path.sexPRS.exclu.males
                                                ,filePrefix
                                                ,"part2",".tsv"))

ExportFileTabSeparated(data=males.base.part3
                       ,output.file.path=paste0(output.folder.path.sexPRS.exclu.males
                                                ,filePrefix
...
# file.copy("PRS_UKB_201711_step17-07-03_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup7-adults-nicotine-dependence-and-more-diagnoses_sex-PRS-int-exclude_one-sex-males.R")
