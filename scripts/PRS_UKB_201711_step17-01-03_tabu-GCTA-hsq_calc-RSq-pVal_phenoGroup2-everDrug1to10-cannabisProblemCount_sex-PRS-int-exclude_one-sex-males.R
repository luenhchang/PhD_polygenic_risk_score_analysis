#-------------------------------------------------------------------------------------------------
# Program       : PRS_UKB_201711_step17-01-03_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup2-everDrug1to10-cannabisProblemCount_sex-PRS-int-exclude_one-sex-males.R
# Modified from : PRS_UKB_201711_step17-01-02_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup2-everDrug1to10-cannabisProblemCount_sex-PRS-int-exclude_one-sex-females.R
...
# Sys.Date()  History
#----------------------------------------------------------------------------------------
# 2018-12-09  Exported the 3 files above
#----------------------------------------------------------------------------------------
...
for (i in 1:length(fileList.sexPRS.exclu.males)){
  fileName.sexPRS.exclu.males <- fileList.sexPRS.exclu.males[i]
  print(paste0("=================================iteration",i,"========================"))
  GCTA.output.sexPRS.exclu.males <- scan(file=paste0(input.folder.path.sexPRS.exclu.males
                                               ,fileName.sexPRS.exclu.males)
                                   ,sep="\t", what="")
  #GCTA.output.sexPRS.inclu <- scan(file=paste0(input.folder.path.sexPRS.inclu,fileName.sexPRS.inclu),sep="\t", what="")
}

# Iteration/file 1-40 have 61 items read; iteration 41-400 have 63 items read
...
fileList.sexPRS.exclu.males.keep <- grep(list.files(path=input.folder.path.sexPRS.exclu.females,pattern = ".hsq")
                                           ,pattern = "CUD"
                                           ,inv = TRUE
                                           ,value = TRUE) # length(fileList.sexPRS.exclu.males.keep) 400 length(fileList.sexPRS.exclu.keep) 400

# Check number of items read again. Each file now has same number of items (63)
for (i in 1:length(fileList.sexPRS.exclu.males.keep)){
  fileName <- fileList.sexPRS.exclu.males.keep[i]
...
# 62,63 30    covar2    (nSEX estimate, SE)
# 64,65 31    covar3    (ImpCov estimate, SE)
#--------------------------------------------------------------
...
data.pheno.group2.males$wave_dup <- data.pheno.group2.males$wave
...
for (i in 1:length(fileList.sexPRS.exclu.males.keep)){
  ...
  # Calculate proportion of phenotypic variance explained by each of the 16 fixed effects
  ...
  for (fixEffect in allFixedEffectVar){
    ...
    phenotype.SD <- sd(x=data.pheno.group2.males[,depVarName],na.rm= T) # Get SD of a target phenotype from phenotype file
    fixed.effect.SD <- sd(data.pheno.group2.males[,fixEffect],na.rm= T) # Get SD of the fixed effect from phenotype file
    # Calculate R2
...
ExportFileTabSeparated(data=males.base.part1
                       ,output.file.path=paste0(output.folder.path.sexPRS.exclu.males,filePrefix,"part1_variance",".tsv"))

ExportFileTabSeparated(data=males.base.part2
                       ,output.file.path=paste0(output.folder.path.sexPRS.exclu.males,filePrefix,"part2",".tsv"))

ExportFileTabSeparated(data=males.base.part3
                       ,output.file.path=paste0(output.folder.path.sexPRS.exclu.males,filePrefix,"part3_fixedEffects",".tsv"))

# Copy this script for similar jobs
#setwd("/mnt/backedup/home/lunC/scripts/PRS_UKB_201711/")
#file.copy("PRS_UKB_201711_step17-01-02_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup2-everDrug1to10-cannabisProblemCount_sex-PRS-int-include.R")
