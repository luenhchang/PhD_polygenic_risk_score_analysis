#----------------------------------------------------------------------------------------------
# Program       : PRS_UKB_201711_step17-03-01_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup5-diagMD-diagSU_sex-PRS-int-exclude_all-sexes.R
# Modified from : 
...
# Type File
#-----------------------------------------------------------------------------------------------------
# Input paste0(input.folder.path.sexPRS.exclu.allSexes,"*.hsq") 840 files
...
# 2018-12-06  Exported the 3 folders and 3 files above
...
# Read *.hsq files 
fileList.sexPRS.exclu.allSexes <- list.files(path=input.folder.path.sexPRS.exclu.allSexes,pattern = ".hsq") # length(fileList.sexPRS.exclu.allSexes) 840 files
...
# Iteration 1-840 have 67 items read. Items scanned from GCTA hsq, their corresponding line number in the hsq, corresponding variable in the input file for GCTA, and values in the line of hsq 
...
for (i in 1:length(fileList.sexPRS.exclu.allSexes)){
  ...
  GCTAOut <- scan(file=paste0(input.folder.path.sexPRS.exclu.allSexes,fileName),sep="\t", what="") # Read 67 items
  ...
  categoricalCovars.allSexes=c("wave","nSEX","impCov")
  
  # Names of the fixed effects in order
  allCovariates.allSexes <- c(inputFileNamePart3.3,quantitativCovars.allSexes,categoricalCovars.allSexes)
  ...
  dfPart3= data.frame(phenotype = depVarName
                      ...
                      ,row.names= c("intercept",allCovariates.allSexes)
...
#file.copy("PRS_UKB_201711_step17-03-01_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup5-diagMD-diagSU_sex-PRS-int-exclude_all-sexes.R","PRS_UKB_201711_step17-03-02_tabu-GCTA-hsq_calc-RSq-pVal_phenoGroup5-diagMD-diagSU_sex-PRS-int-exclude_one-sex-females.R")
